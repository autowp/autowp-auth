...
func NewMemoryTokenStore() (oauth2server.TokenStore, error) {
	return NewFileTokenStore(":memory:")
}

// NewFileTokenStore create a token store instance based on file
func NewFileTokenStore(filename string) (oauth2server.TokenStore, error) {
...
func (ts *TokenStore) Create(info oauth2server.TokenInfo) error {
	ct := time.Now()
...
func (ts *TokenStore) getData(key string) (oauth2server.TokenInfo, error) {
	var ti oauth2server.TokenInfo
...
func (ts *TokenStore) GetByCode(code string) (oauth2server.TokenInfo, error) {
	return ts.getData(code)
}

// GetByAccess use the access token for token information data
func (ts *TokenStore) GetByAccess(access string) (oauth2server.TokenInfo, error) {
...
func (ts *TokenStore) GetByRefresh(refresh string) (oauth2server.TokenInfo, error) {
